...
- 支持影片搜索、媒体库筛选和排序。
- 支持分页浏览和手动刷新媒体库。
- 海报墙、详情页和设置页采用跟随屏幕刷新率的像素级弹性滚动，海报墙滚动时带轻量动态模糊。

### 电视剧与单集

- 电视栏目按整部剧集显示一张总海报，不再把每一集铺开在海报墙中。
- 点击总海报后按季显示单集列表，包含 `S01E01`/集数、单集名称、时长和观看状态。
- 选择单集后可查看真实文件路径、视频规格，并直接交给 PotPlayer 播放。
- 本地/NAS 扫描支持识别 `S01E02`、`第02集`、`剧名-02-集名` 等常见文件名并自动归并。
...
```text
Windows 路径：\\192.168.x.xx\Movies\007\movie.mkv
```
...
Windows 路径前缀：\\192.168.xx.xx\Movies\
```
